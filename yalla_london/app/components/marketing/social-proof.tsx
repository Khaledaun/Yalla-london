    ...
    <div className={`space-y-3 ${className}`} dir={isArabic ? 'rtl' : 'ltr'}>
      ...
      {showViewers && displayStats.active_viewers > 0 && (
        ...
        >
          ...
          <span className="text-yl-gray-500">{t.viewingNow}</span>
        </motion.div>
      )}
      ...
      {showViewers && displayStats.views_week > 0 && (
        <div className="flex items-center gap-2 text-sm text-yl-gray-500">
          <Eye className="h-4 w-4 text-yl-red" />
          <span className="font-semibold text-yl-charcoal">{displayStats.views_week.toLocaleString()}</span>
