...
export function SiteProvider({ children }: { children: React.ReactNode }) {
  ...
  // Load saved site preference from localStorage
  useEffect(() => {
    const savedSiteId = localStorage.getItem('admin_current_site_id')
    if (savedSiteId) {
      const savedSite = MOCK_SITES.find(s => s.id === savedSiteId)
      if (savedSite) {
  ...
  const getSiteBySlug = useCallback((slug: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.sites && Array.isArray(data.sites)) {
          ...
          const updatedCurrentSite = data.sites.find((s: Site) => s.id === currentSite.id)
...
export function useSiteData<T>(
  ...
  return { data, isLoading: isLoading || siteLoading, error, siteId: currentSite.id }
}
