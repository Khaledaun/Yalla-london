...
describe('SEO Meta Service', () => {
  ...
  const testData: SEOMetaData = {
    ...
    hreflangAlternates: { en: 'https://example.com/test-page', ar: 'https://example.com/ar/test-page' },
    seoScore: 85
  };
  ...
  describe('saveSEOMeta', () => {
    ...
    it('should handle structured data correctly', async () => {
      const dataWithStructuredData = {
        ...testData,
        structuredData: { '@type': 'Article', headline: 'Test Article', author: 'Test Author' }
      };
      
      const result = await seoMetaService.saveSEOMeta(testPageId, dataWithStructuredData);
      
      expect(result.structuredData).toEqual(dataWithStructuredData.structuredData);
    });
    ...
    it('should handle hreflang alternates correctly', async () => {
      ...
      const result = await seoMetaService.saveSEOMeta(testPageId, dataWithHreflang);
      
      expect(result.hreflangAlternates).toEqual(dataWithHreflang.hreflangAlternates);
    });
  });
  ...
  describe('getSEOMetaByURL', () => {
    ...
    it('should return null for non-existent URL', async () => {
      const result = await seoMetaService.getSEOMetaByURL('https://example.com/non-existent');
  ...
  describe('deleteSEOMeta', () => {
    ...
    it('should handle deletion of non-existent data gracefully', async () => {
      ...
      await expect(seoMetaService.deleteSEOMeta('non-existent-page')).resolves.not.toThrow();
    });
  });
  ...
  describe('calculateSEOScore', () => {
    ...
    it('should give higher score for complete data', () => {
      const completeData: SEOMetaData = {
        ...
        hreflangAlternates: { en: 'https://example.com/en', ar: 'https://example.com/ar' }
      };
