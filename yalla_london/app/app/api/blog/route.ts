 ...
 * GET /api/blog
 *
 * Public read-only blog API. Returns published posts for the given site.
 * No auth required — only returns published, non-deleted posts.
 *
 * Query params:
 *   ?limit=N      — max posts to return (default: 20, max: 50)
 *   ?offset=N     — pagination offset (default: 0)
 *   ?site=ID      — filter by site ID (default: yalla-london)
...
export async function GET(request: NextRequest) {
  try {
    ...
    const siteId = searchParams.get("site") || "yalla-london";
    ...
    const [posts, total] = await Promise.all([
      prisma.blogPost.findMany({
        ...
        select: {
          ...
          excerpt_en: true,
          excerpt_ar: true,
          content_en: false, // Don't send full content in list
          content_ar: true, // Include to verify bilingual content exists
          featured_image: true,
          published: true,
          tags: true,
          meta_title_en: true,
