...
const SEED_NEWS = [
  {
    ...
    meta_description_ar: "يربط خط إليزابيث مطار هيثرو بوسط لندن في أقل من 30 دقيقة. كل ما يحتاج الزوار معرفته.",
...
function filterSeedData(category: string | undefined, majorOnly: boolean, limit: number) {
  return SEED_NEWS.filter((item) => {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const items = await prisma.newsItem.findMany({
      where,
      orderBy: [
        { published_at: "desc" }, // Recency first — old "BREAKING" stories should not pin to top forever
        { is_major: "desc" },
      ],
      select,
      take: limit,
    ...
    console.warn(
      "[news-api] Database unavailable, falling through to seed data:",
