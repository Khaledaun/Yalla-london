...
const seoSection = async (
  ...
  try {
    ...
    const recentReports = await prisma.sEOReport.findMany({
      ...
      take: 3,
      select: { id: true, report_type: true, health_score: true, created_at: true },
    });

    if (recentReports.length > 0) {
      const latest = recentReports[0];
      const ageHours = Math.round((Date.now() - new Date(latest.created_at).getTime()) / (1000 * 60 * 60));
      results.push(pass("seo-reports", "SEO Reports", `${recentReports.length} report(s) — latest: ${ageHours}h ago (score: ${latest.health_score || "N/A"})`, "SEO reports are generated by the weekly orchestrator. They track your site's search health over time and flag issues that need attention."));
    } else {
