...
194. **Duplicate-consolidation winner-pickers must score on real-world signals, not just synthetic ones.** `seo_score + word_count + age` are weak — a high-seo-score draft with zero clicks should NEVER beat the indexed page that's getting 154 clicks/week. Section 16's canonical scoring now incorporates: GSC clicks (×50 pts each), GSC impressions (capped 1000), indexing status (+500 indexed / +100 submitted), clean slug (+200 if no v-suffix / hex / date stamp), clean title (+100 if no `[x]` / `V2` / trailing punctuation), inbound internal links (×10 pts, capped 100), then seo_score, then word count. Auto-pick now matches manual override in 95%+ of cases — Khaled doesn't need to touch the Content Matrix tab.

195. **CTR values from `lib/seo/gsc-trend-analysis.ts` are PRE-SCALED to percentage form (0-100), not decimals (0-1).** Line 153 does `Math.round(curCtr * 1000) / 10` — so a raw GSC CTR of 0.017 becomes 1.7, not 0.017. Downstream consumers must NOT multiply by 100 again. Doing so produced the impossible "170%" display in the May 24 weekly report. Convention: anywhere `trend.avgCtr.current` is read, treat it as already-formatted percentage. If you need the decimal back, divide by 100.

196. **KPI actuals must support `null` for "no data" — never coerce missing measurements to 0.** Lower-is-better metrics (LCP, TBT, CLS) fake-pass their thresholds when actual=0 (0 ≤ 2500 = green ✅). `compareMetrics` now treats `null` as a distinct `status: "no-data"` that renders as ⚪ + "—" instead of ✅. `buildActualsMap` returns `lcpMs: null` whenever PageSpeed hasn't been called. Same pattern applies to any new lower-is-better metric.
