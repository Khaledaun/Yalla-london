...
function getSiteInfo(siteId?: string) {
  ...
  const serviceDesc = isYacht
    ...
    : 'a bilingual (English/Arabic) luxury London travel guide providing curated, editorially independent content'
  const serviceDescAr = isYacht
    ? 'منصة وساطة لتأجير اليخوت الفاخرة ثنائية اللغة (إنجليزي/عربي) تغطي البحر الأبيض المتوسط'
    : 'دليل سفر فاخر ثنائي اللغة (إنجليزي/عربي) للندن يقدم محتوى تحريري مستقل ومختار'
...
function getContent(siteId?: string) {
  ...
  en: {
    ...
    intro: `Welcome to ${s.name}, operated by Zenitha.Luxury LLC ("Company", "we", "us", or "our"). By accessing and using our website at ${s.domain} (the "Website"), you agree to be bound by these Terms of Use. If you do not agree with any part of these terms, please do not use our Website. These terms apply to all visitors, users, and subscribers regardless of language preference.`,

    aboutTitle: 'About Our Service',
    aboutIntro: `${s.name} is ${s.serviceDesc}:`,
    aboutItems: s.isYacht ? [
      'We provide curated yacht charter listings across the Mediterranean, including motor yachts, sailing yachts, catamarans, and superyachts. Our team verifies each listing for accuracy, safety standards, and guest experience quality',
      'We publish destination guides, itinerary recommendations, and charter planning advice covering the Mediterranean\'s most sought-after cruising grounds',
      'We maintain a comprehensive charter planner with AI-powered yacht matching to help guests find the perfect vessel for their group size, budget, and preferences',
      'We connect guests with vetted charter brokers and operators, facilitating inquiries and providing transparent pricing information',
      'We offer digital charter planning resources including destination guides, packing lists, and itinerary templates',
      'All content is available in both English and Arabic, reflecting our mission to serve Arabic-speaking luxury travellers and the broader international yachting community',
    ] : [
    ...
    affiliateIntro: `${s.name} earns revenue through affiliate partnerships. We believe in full transparency about how this works:`,
    affiliateItems: [
      s.isYacht
        ? 'We participate in affiliate programmes with Boatbookings, Click&Boat, CharterWorld, Burgess, Yacht Charter Fleet, Booking.com, and select yacht charter and luxury travel platforms. When you click an affiliate link on our Website and complete a purchase or booking on the partner platform, we earn a commission from that partner'
    ...
    accuracyItems: [
      ...
      `${s.name} is not liable for any losses, disappointments, or inconveniences arising from outdated or inaccurate information on our Website. We provide content for informational and inspirational purposes`,
    ],
  ...
  ar: {
    ...
    intro: `مرحباً بك في ${s.nameAr}، الذي تديره شركة Zenitha.Luxury LLC ("الشركة" أو "نحن" أو "لنا" أو "خاصتنا"). بالوصول إلى موقعنا واستخدامه على ${s.domain} ("الموقع")، فإنك توافق على الالتزام بشروط الاستخدام هذه. إذا كنت لا توافق على أي جزء من هذه الشروط، يرجى عدم استخدام موقعنا. تنطبق هذه الشروط على جميع الزوار والمستخدمين والمشتركين بغض النظر عن تفضيل اللغة.`,

    aboutTitle: 'حول خدمتنا',
    aboutIntro: `${s.nameAr} ${s.serviceDescAr}:`,
    aboutItems: s.isYacht ? [
      'نقدم قوائم مختارة لتأجير اليخوت عبر البحر الأبيض المتوسط، بما في ذلك اليخوت الآلية واليخوت الشراعية والقوارب ذات البدنين واليخوت الفاخرة الكبرى. يتحقق فريقنا من كل قائمة من حيث الدقة ومعايير السلامة وجودة تجربة الضيوف',
      'ننشر أدلة الوجهات وتوصيات خطوط الرحلات ونصائح تخطيط الرحلات البحرية التي تغطي أكثر مناطق الإبحار المرغوبة في البحر الأبيض المتوسط',
      'نحافظ على مخطط رحلات بحرية شامل مع مطابقة يخوت مدعومة بالذكاء الاصطناعي لمساعدة الضيوف في العثور على السفينة المثالية لحجم مجموعتهم وميزانيتهم وتفضيلاتهم',
      'نربط الضيوف بوسطاء ومشغلي رحلات بحرية موثوقين، لتسهيل الاستفسارات وتقديم معلومات تسعير شفافة',
      'نقدم موارد رقمية لتخطيط الرحلات البحرية بما في ذلك أدلة الوجهات وقوائم التعبئة وقوالب خطوط الرحلات',
      'جميع المحتوى متاح باللغتين الإنجليزية والعربية، مما يعكس مهمتنا في خدمة المسافرين الناطقين بالعربية ومجتمع اليخوت الدولي الأوسع',
    ] : [
    ...
    accuracyItems: [
      ...
      `${s.nameAr} غير مسؤولة عن أي خسائر أو خيبات أمل أو مضايقات ناتجة عن معلومات قديمة أو غير دقيقة على موقعنا. نقدم المحتوى لأغراض إعلامية وإلهامية`,
    ],
...
export default function TermsOfUse({ siteId }: { siteId?: string }) {
