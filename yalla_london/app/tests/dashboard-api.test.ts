...
describe('/api/admin/dashboard', () => {
  ...
  it('should handle time range parameter', async () => {
    const request = new NextRequest('http://localhost:3000/api/admin/dashboard?timeRange=30d');
  ...
  it('should show connected status when analytics config exists', async () => {
    mockPrisma.apiSettings.findFirst
      .mockResolvedValueOnce({ key_value: 'GA4_ID_123', test_status: 'success' })
      .mockResolvedValueOnce({ key_value: 'GSC_ID_456', test_status: 'success' });
