...
function validateTopicProposal(topic: any) {
  ...
  if (
    !topic.authority_links_json ||
    topic.authority_links_json.length < 3 ||
    topic.authority_links_json.length > 4
  ) {
  ...
  if (topic.authority_links_json) {
    topic.authority_links_json.forEach((link: any, index: number) => {
      if (!link.url || !link.title || !link.sourceDomain) {
        errors.push(
          `Authority link ${index + 1} missing required fields (url, title, sourceDomain)`,
        );
      }
  ...
  if (
    topic.confidence_score !== undefined &&
    (topic.confidence_score < 0 || topic.confidence_score > 1)
  ) {
...
function shouldTriggerBacklinkOffers(
  indexedPages: number,
  minPages: number = 40,
): boolean {
  ...
  test("should reject topic with malformed authority links", () => {
    ...
    expect(
      result.errors.some((error) =>
        error.includes("Authority link 1 must have valid URL"),
      ),
    ).toBe(true);
    expect(
      result.errors.some((error) =>
        error.includes("Authority link 2 missing required fields"),
      ),
    ).toBe(true);
  });
...
describe("SEO Score Validation Business Rules", () => {
  test("should validate correct SEO scores", () => {
    ...
    expect(validateSeoScore(55)).toEqual({
      isValid: true,
      level: "needs_improvement",
    });
    expect(validateSeoScore(0)).toEqual({
      isValid: true,
      level: "needs_improvement",
    });
    expect(validateSeoScore(100)).toEqual({
      isValid: true,
      level: "excellent",
    });
  });
