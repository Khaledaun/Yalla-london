...
export default function PurchasesPage() {
  ...
  const statusIcon = (status: string) => {
    switch (status) {
      case 'COMPLETED': return <Check className="w-4 h-4 text-yl-charcoal" />
      case 'PENDING': return <Clock className="w-4 h-4 text-yl-gold" />
      default: return <AlertCircle className="w-4 h-4 text-yl-red" />
    }
  }
  ...
  return (
    <div className="min-h-screen bg-yl-cream" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-center p-4 pointer-events-none">
        <nav className="flex items-center justify-between px-6 py-3 bg-white/95 backdrop-blur-xl rounded-full shadow-lg w-full max-w-4xl pointer-events-auto border border-yl-gray-200">
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link href="/shop" className="text-sm font-medium text-yl-gray-500 hover:text-yl-charcoal">Shop</Link>
            <Link href="/shop/purchases" className="text-sm font-medium text-yl-charcoal">My Purchases</Link>
          </div>
        </nav>
      </div>
      ...
      <div className="pt-28 pb-16 max-w-2xl mx-auto px-6">
        ...
        <h1 className="text-3xl font-bold text-yl-charcoal mb-2">
        ...
        <form onSubmit={lookupPurchases} className="flex gap-3 mb-8">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-yl-gray-500" />
            <input
              type="email"
              required
              ...
              className="w-full pl-10 pr-4 py-3 border border-yl-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-yl-red/20 focus:border-yl-red"
            />
          </div>
          <button
            type="submit"
            disabled={loading}
            className="px-6 py-3 bg-yl-red hover:bg-yl-red text-white font-semibold rounded-xl transition-colors disabled:opacity-50"
          >
        ...
        {error && (
          <div className="p-4 bg-yl-red/5 text-yl-red rounded-xl mb-6">{error}</div>
        )}
          ...
          <>
            {purchases.length === 0 ? (
              <div className="text-center py-12">
                <Package className="w-16 h-16 mx-auto mb-4 text-yl-gray-500" />
                <h2 className="text-xl font-bold text-yl-charcoal mb-2">
                ...
                <Link href="/shop" className="text-yl-red hover:underline font-medium">
              ...
              <div className="space-y-4">
                {purchases.map((purchase) => (
                  <div key={purchase.id} className="bg-white rounded-xl border border-yl-gray-200 shadow-sm p-5">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-semibold text-yl-charcoal truncate">
                            {isRTL ? (purchase.product.name_ar || purchase.product.name_en) : purchase.product.name_en}
                          </h3>
                          {statusIcon(purchase.status)}
                        </div>
                        <div className="flex items-center gap-3 text-sm text-yl-gray-500">
                      ...
                      {purchase.downloadUrl && purchase.downloadsRemaining > 0 && (
                        <a
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-yl-red hover:bg-yl-red text-white font-medium rounded-lg transition-colors flex-shrink-0"
                        >
