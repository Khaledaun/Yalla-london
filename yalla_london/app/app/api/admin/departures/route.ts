...
const CRON_DEFS: CronDef[] = [
  ...
  { path: '/api/cron/content-builder',        schedule: '*/15 * * * *',       label: 'Content Builder (15min)',   icon: '🏗️', type: 'cron', category: 'content',      description: 'Advances article drafts through the 8-phase pipeline. Full 53s budget dedicated to AI phase execution.', feedsInto: 'Content Selector' },
  { path: '/api/cron/content-builder-create', schedule: '7,37 * * * *',       label: 'Draft Creator (30min)',     icon: '📝', type: 'cron', category: 'content',      description: 'Creates new EN+AR draft pairs from topic queue. DB-only, no AI calls. Separated so builder gets full budget for phases.', feedsInto: 'Content Builder' },
  ...
  { path: '/api/affiliate/cron/sync-commissions',  schedule: '0 4 * * *',          label: 'CJ Commission Sync',    icon: '💵', type: 'cron', category: 'content',      description: 'Fetches commission data from CJ for the last 7 days. Updates revenue dashboard with real earnings.' },
  { path: '/api/affiliate/cron/discover-deals',    schedule: '0 6 * * *',          label: 'CJ Deal Discovery',     icon: '🏷️', type: 'cron', category: 'content',      description: 'Searches CJ product catalog for new deals and price drops relevant to each site destination.' },
  { path: '/api/affiliate/cron/refresh-links',     schedule: '0 12 * * *',         label: 'CJ Link Refresh',       icon: '🔗', type: 'cron', category: 'content',      description: 'Refreshes affiliate tracking links from CJ for all joined advertisers. Deactivates expired links.' },
  ...
  { path: '/api/cron/content-auto-fix-lite',  schedule: '0 */4 * * *',        label: 'Auto-Fix Lite',             icon: '🔧', type: 'cron', category: 'content',      description: 'Fast DB-only fixes: stuck draft recovery, heading hierarchy, meta description trims. Every 4 hours.',          feedsInto: 'Content Builder' },
  ...
  { path: '/api/cron/perplexity-scheduler',  schedule: '0 */2 * * *',        label: 'Perplexity Scheduler',      icon: '🔮', type: 'cron', category: 'ai',            description: 'Processes due Perplexity Computer schedules. Creates tasks from templates for recurring research, audits, and monitoring.',  feedsInto: 'Perplexity Tasks' },
];
