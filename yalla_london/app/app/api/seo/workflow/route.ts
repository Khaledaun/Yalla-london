    ...
    if (action === "verify-gsc" || action === "status") {
      ...
      return NextResponse.json({
        ...
        endpoints: {
          ...
          runOptimization:
            'POST /api/seo/workflow { "workflow": "optimization", "urls": [...] }',
          runResearch:
            'POST /api/seo/workflow { "workflow": "research", "keyword": "...", "locale": "en" }',
          auditContent:
            'POST /api/seo/workflow { "action": "audit", "urls": [...] }',
          verifyIndexing:
            'POST /api/seo/workflow { "action": "verify_indexing", "urls": [...], "forceSubmit": true }',
        },
      });
    }
      ...
      availableWorkflows: [
        {
          ...
          name: "Full SEO Audit",
          description:
            "Comprehensive SEO audit including GSC verification, content audit, and indexing check",
        },
        {
          id: "indexing",
          name: "Indexing Check",
          description:
            "Verify indexing status of all URLs and submit unindexed to search engines",
        },
        {
          id: "optimization",
          name: "Content Optimization",
          description:
            "Analyze and optimize content for SEO using parallel agents",
        },
        {
          id: "research",
          name: "SEO Research",
          description:
            "Structured SEO research with verification methodology (from zenobi-us/dotfiles)",
        },
      ],
      skills: [
        {
          ...
          description:
            "Parallel research methodology with source verification and confidence levels",
        },
        {
          id: "parallel-seo-agents",
          name: "Parallel SEO Agents",
          description:
            "Dispatch multiple specialized agents for independent SEO tasks",
        },
      ],
      endpoints: {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === "audit" && urls) {
      ...
      return NextResponse.json({
        ...
        summary: {
          ...
          avgSeoScore:
            auditResults.reduce((sum, a) => sum + a.seoScore, 0) /
            auditResults.length,
        },
      });
    }
    ...
    if (action === "verify_indexing") {
      const urlsToCheck =
        urls ||
        (await getAllIndexableUrls())
          .filter((u) => u.includes("/blog/"))
          .slice(0, 20);
      const results = await verifyAndSubmitForIndexing(
        urlsToCheck,
        forceSubmit,
      );
      return NextResponse.json({
