...
`;
    ...
    // Test IndexNow
    const urls = (await getAllIndexableUrls()).slice(0, 5);
...
`;
    ...
    const testUrl = (await getAllIndexableUrls())[0];
    ...
    const status = debugResult.success
      ? await gscApi.checkIndexingStatus(testUrl)
      : null;
...
`;
    ...
    const urls = (await getAllIndexableUrls()).slice(0, 5); // Test with 5 URLs
