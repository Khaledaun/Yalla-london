...
export function TrustBadges({
  ...
  if (variant === 'minimal') {
    return (
      ...
      >
        ...
        <div className="flex items-center gap-1 text-yl-gray-500 text-sm">
  ...
  return (
    ...
    >
      ...
      {/* Payment Methods */}
      {showPayments && (
        <div className="border-t border-yl-gray-200 pt-6">
          <p className="text-center text-sm text-yl-gray-500 mb-4">{t.paymentMethods}</p>
          <div className="flex flex-wrap items-center justify-center gap-4">
            {PAYMENT_METHODS.map((method) => (
              <div
                key={method.name}
                className="w-12 h-8 bg-white border border-yl-gray-200 rounded flex items-center justify-center"
                title={method.alt}
              >
                {/* Using placeholder styling since actual SVGs might not exist */}
                <span className="text-xs font-medium text-yl-gray-500">{method.name.slice(0, 2)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {showPartners && (
        <div className="border-t border-yl-gray-200 pt-6">
          <p className="text-center text-sm text-yl-gray-500 mb-4">{t.trustedPartners}</p>
          <div className="flex flex-wrap items-center justify-center gap-6">
            {PARTNER_LOGOS.map((partner) => (
              ...
              >
                <div className="h-8 flex items-center">
                  <span className="text-sm font-medium text-yl-gray-500">{partner.name}</span>
                </div>
              </div>
            ))}
          </div>
...
function TrustBadge({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex items-center gap-2 text-yl-gray-500">
...
function SecurityBadge({
  ...
  return (
    <div className="flex items-start gap-3 max-w-[200px]">
      ...
      <div>
        <div className="font-semibold text-yl-charcoal">{title}</div>
        <div className="text-sm text-yl-gray-500">{description}</div>
      </div>
    </div>
  );
}
...
export function FooterTrustStrip({ locale = 'en' }: { locale?: 'en' | 'ar' }) {
  ...
  return (
    <div
      className="bg-yl-cream border-y border-yl-gray-200 py-6"
      dir={isArabic ? 'rtl' : 'ltr'}
    >
