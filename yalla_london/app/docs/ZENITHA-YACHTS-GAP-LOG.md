...
| ID | Area | Severity | Description | Status | Fix Applied | File(s) Affected |
|----|------|----------|-------------|--------|-------------|-----------------|
| ZY-M01 | Design System | MEDIUM | Yacht admin pages use hardcoded Tailwind color classes (e.g., `bg-blue-600`, `text-gray-700`) instead of Zenitha design tokens (`var(--z-navy)`, `var(--z-gold)`). Creates visual inconsistency between public yacht pages and admin yacht pages | FIXED | Removed dead `statusColors` Tailwind classes from analytics page (only file affected). All 8 admin yacht pages already use Clean Light admin-ui inline styles with hex colors matching Zenitha brand palette. Bar chart uses inline hex colors aligned with Zenitha tokens. | `app/admin/yachts/analytics/page.tsx` |
...
| ZY-M07 | Schema Consistency | MEDIUM | Design system field naming inconsistency — some models and APIs use `site` (string) while others use `siteId` (string). Both refer to the same concept but create confusion in API contracts | FIXED | False positive — full audit confirmed 100% consistency: all 7 yacht Prisma models use `siteId`, all 11 admin API routes use `siteId` (query param + body), all 4 public API routes use `siteId` (via `x-site-id` header), all 7 admin pages send `siteId` in fetch calls. No naming inconsistency exists. | N/A (no changes needed) |
...
| ZY-M10 | i18n | MEDIUM | Arabic route handling not optimized for yacht site — `/ar/` prefix routing works but yacht-specific content (fleet specs, itinerary descriptions, inquiry forms) has no Arabic translation pipeline or bilingual field population strategy | DEFERRED | Strategic gap — Arabic content requires: (1) bilingual fields on Yacht/YachtDestination/CharterItinerary models (name_ar, description_ar), (2) seed data with Arabic translations, (3) yacht pages reading `useLanguage()` and displaying `_ar` variants. Infrastructure (middleware `/ar/` routing, `useLanguage()` provider, `LanguageSwitcher`) is already in place. Deferred to post-launch — English-first launch then Arabic content phase. | `prisma/schema.prisma`, yacht page components |
