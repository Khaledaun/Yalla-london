...
export default function AutopilotPage() {
  ...
  const toggleGlobalStatus = () => {
    ...
    setTasks((prev) =>
      prev.map((task) => ({
        ...task,
        status: globalStatus === 'running' ? 'paused' : task.status === 'paused' ? 'running' : task.status,
      }))
    );
  };

  const runningTasks = tasks.filter((t) => t.status === 'running').length;
  const totalItemsProcessed = tasks.reduce((sum, t) => sum + t.stats.itemsProcessed, 0);
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="text-2xl font-bold">{totalItemsProcessed.toLocaleString()}</div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    ...
                    <span className="text-sm text-gray-500">
                      {task.sites.length === 0 ? 'All sites' : task.sites.join(', ')}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center bg-gray-50 rounded-lg p-2">
                    <div>
                      <div className="text-sm font-medium">{task.stats.totalRuns}</div>
                      <div className="text-xs text-gray-500">Runs</div>
                    </div>
                    <div>
                      <div className="text-sm font-medium">{task.stats.successRate}%</div>
                      <div className="text-xs text-gray-500">Success</div>
                    </div>
                    <div>
                      <div className="text-sm font-medium">{task.stats.itemsProcessed}</div>
        ...
        <div className="mt-8 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100">
          ...
          <div className="grid md:grid-cols-4 gap-4 text-sm">
            ...
            <div className="flex items-start gap-2">
              ...
              <p className="text-blue-700">
                Enable autopilot and let it run!
              </p>
            </div>
          </div>
        </div>
...
const mockTasks: AutopilotTask[] = [
  {
    ...
    nextRun: 'In 4 hours',
    sites: [],
    config: { articlesPerRun: 5, categories: ['resorts', 'guides'] },
    stats: { totalRuns: 120, successRate: 98.5, itemsProcessed: 580 },
  },
  {
    id: 'task-2',
    ...
    nextRun: 'In 6 hours',
    sites: [],
    config: { optimizeImages: true, updateMeta: true },
    stats: { totalRuns: 45, successRate: 100, itemsProcessed: 1250 },
  },
  {
    id: 'task-3',
    ...
    nextRun: null,
    sites: ['arabaldives'],
    config: { platforms: ['twitter', 'instagram'], includeImages: true },
    stats: { totalRuns: 85, successRate: 95.2, itemsProcessed: 340 },
  },
  {
    id: 'task-4',
    ...
    nextRun: 'In 2 days',
    sites: [],
    config: { template: 'weekly-digest', segments: ['all'] },
    stats: { totalRuns: 12, successRate: 100, itemsProcessed: 8500 },
  },
  {
    id: 'task-5',
    ...
    nextRun: 'In 2 days',
    sites: [],
    config: { includeRevenue: true, compareToLastWeek: true },
    stats: { totalRuns: 8, successRate: 100, itemsProcessed: 24 },
  },
];

const mockLogs: AutopilotLog[] = [
  {
    id: 'log-1',
    taskId: 'task-1',
    taskName: 'Daily Content Generation',
    type: 'content',
    status: 'success',
    message: 'Generated 5 new articles for Arabaldives',
    timestamp: '2 hours ago',
    details: 'Topics: Resort Reviews, Travel Tips, Honeymoon Guide',
  },
  {
    id: 'log-2',
    taskId: 'task-2',
    taskName: 'SEO Meta Optimization',
    type: 'seo',
    status: 'success',
    message: 'Optimized 45 pages across all sites',
  ...
  },
  {
    ...
    type: 'content',
    status: 'success',
    message: 'Generated 5 new articles for Gulf Maldives',
    timestamp: '8 hours ago',
  },
  {
    id: 'log-4',
    taskId: 'task-3',
    taskName: 'Social Media Posts',
    type: 'social',
    status: 'error',
    message: 'Failed to post to Instagram',
    timestamp: 'Yesterday',
    details: 'Error: API rate limit exceeded. Retry in 1 hour.',
  },
  {
    id: 'log-5',
    taskId: 'task-4',
    ...
    type: 'email',
    status: 'success',
    message: 'Sent newsletter to 2,340 subscribers',
