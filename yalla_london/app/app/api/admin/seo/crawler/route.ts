    ...
    for (const url of urls) {
      try {
        ...
        results.push({
          url,
          status: 'error',
          issues: [`Crawl error: ${error instanceof Error ? error.message : 'Unknown error'}`],
          score: 0,
          lastCrawled: new Date().toISOString()
        })
      }
...
function determineStatusAndIssues(analysis: SEOAnalysis, score: number): { status: 'success' | 'warning' | 'error', issues: string[] } {
  const issues: string[] = []
  
  if (!analysis.title.present) {
    issues.push('Missing page title')
  } else if (!analysis.title.optimal) {
    issues.push(`Title length ${analysis.title.length} (optimal: 30-60 characters)`)
  }
  
  if (!analysis.metaDescription.present) {
    issues.push('Missing meta description')
  } else if (!analysis.metaDescription.optimal) {
    issues.push(`Meta description length ${analysis.metaDescription.length} (optimal: 120-160 characters)`)
  }
...
export async function GET() {
  return NextResponse.json({
    message: 'SEO Crawler API',
    endpoints: {
      POST: 'Crawl websites for SEO analysis',
      parameters: {
