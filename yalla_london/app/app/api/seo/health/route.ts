...
import { validateSEOEnvironment } from "@/lib/seo/validate-seo-env";
...
export async function GET() {
  ...
  try {
    ...
    if (missingKeys.length > 0) {
      reasons.push(
        `Missing required API keys for enabled features: ${missingKeys.join(", ")}`,
      );
      overallOk = false;
    }
    ...
    const featureFlags = {
      ...
      FEATURE_ANALYTICS_DASHBOARD: getFeatureFlagStatus(
        "FEATURE_ANALYTICS_DASHBOARD",
      ),
      FEATURE_MULTILINGUAL_SEO: getFeatureFlagStatus(
        "FEATURE_MULTILINGUAL_SEO",
      ),
      FEATURE_SCHEMA_GENERATION: getFeatureFlagStatus(
        "FEATURE_SCHEMA_GENERATION",
      ),
      FEATURE_SITEMAP_AUTO_UPDATE: getFeatureFlagStatus(
        "FEATURE_SITEMAP_AUTO_UPDATE",
      ),
    };

    // Validate SEO environment variables
    const envValidation = validateSEOEnvironment();
    if (!envValidation.valid) {
      envValidation.warnings.forEach((w) => reasons.push(w));
      overallOk = false;
    }
    ...
    const healthData = {
      ...
      envVars: {
        critical: envValidation.critical.map((v) => ({
          name: v.name,
          present: v.present,
          required: v.required,
        })),
        recommended: envValidation.recommended.map((v) => ({
          name: v.name,
          present: v.present,
        })),
      },
