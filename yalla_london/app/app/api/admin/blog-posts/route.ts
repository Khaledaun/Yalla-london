 ...
 */
export const GET = withTenantAuth(async (request: NextRequest, { db }) => {
  try {
    ...
    // Build where clause (site_id auto-injected by tenant-scoped db)
    const where: any = {};
    ...
    // Fetch blog posts with all related data (auto-scoped by site_id)
...
export const POST = withTenantAuth(
  async (request: NextRequest, { db, siteId }) => {
    try {
      ...
      // Check for duplicate slug (scoped to tenant)
      ...
      const [category, author] = await Promise.all([
        db.category.findFirst({ where: { id: data.category_id } }),
      ...
      // Create the blog post (site_id auto-injected by tenant-scoped db)
