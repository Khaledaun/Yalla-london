...
import { DISCOVERY_REQUIREMENTS, THRESHOLDS, SCORING_WEIGHTS, SEVERITY_DEDUCTIONS, computeGrade } from "./standards";
...
export async function scanSiteDiscovery(
  ...
  // Use getCachedSitemap() (which honors the 24h staleness check) instead of
  // raw Prisma read. If the cache is missing or stale, REGENERATE it
  // synchronously before scanning — otherwise EVERY page on the site gets
  // flagged as "not in sitemap" (HIGH severity, 15-point deduction each)
  // which crashes the discovery score from B → F overnight.
  //
  // Regeneration takes ~5-15s for typical sites; discovery-monitor has a
  // 280s budget so this is well within bounds.
  const sitemapSlugs = new Set<string>();
  try {
    const { getCachedSitemap, regenerateSitemapCache } = await import("@/lib/sitemap-cache");
    let cache = await getCachedSitemap(siteId);
    if (!cache) {
      console.log(`[discovery-scanner] Sitemap cache for ${siteId} missing/stale — regenerating before scan`);
      try {
        await regenerateSitemapCache(siteId);
        cache = await getCachedSitemap(siteId);
      } catch (regenErr) {
        console.warn(
          `[discovery-scanner] Sitemap regen failed for ${siteId}:`,
    ...
    console.warn(
      `[discovery-scanner] Sitemap cache read failed for ${siteId}:`,
...
function analyzePageDiscovery(
  ...
  const textEn = contentEn
    .replace(/<[^>]*>/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  const textAr = contentAr
    .replace(/<[^>]*>/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  ...
  const internalLinks = (
    contentEn.match(
      /href="\/(blog|news|information|guides|hotels|experiences|recommendations|events|about|contact)\//gi,
    ) || []
  ).length;
  const externalLinks = (
    contentEn.match(
      /href="https?:\/\/(?!(?:www\.)?(?:yalla-london|arabaldives|yallariviera|yallaistanbul|yallathailand|zenithayachts)\.com)/gi,
    ) || []
  ).length;
  ...
  const directAnswer =
    /^(?:The |A |An |Yes|No|In \d{4}|London|Dubai|Istanbul|Maldives|Thailand)/i.test(first80Words) &&
    !first80Words.toLowerCase().startsWith("in this");
  const questionH2s = (contentEn.match(/<h2[^>]*>[^<]*\?<\/h2>/gi) || []).length;
  const definitiveStatements = (textEn.match(/\b(?:is|are|costs?|takes?|requires?|includes?|offers?)\b/gi) || [])
    .length;
  const citableData = (
    textEn.match(/(?:£|\$|€)\s*\d+|\d+%|\d{4}(?:\s|-)\d{2}|\d+\s*(?:minutes?|hours?|days?|km|miles?|stars?)/gi) || []
  ).length;
  const listsOrTables = (contentEn.match(/<(?:ul|ol|table)[^>]*>/gi) || []).length;

  const aioEligibility = Math.min(
    100,
    Math.round(
      (directAnswer ? 25 : 0) +
  ...
  const aioStatus: AIOStatus = aioEligibility >= 70 ? "eligible" : aioEligibility >= 40 ? "not_eligible" : "unknown";
  ...
  if (indexStatus === "never_submitted") {
    issues.push(
      makeIssue(
        ...
        {
          id: "submit-indexnow",
          label: "Submit to All Engines",
          description: "Submit via IndexNow to Bing, Yandex, and Google",
          endpoint: "/api/admin/discovery",
          payload: { action: "submit_page", slug: post.slug, siteId },
    ...
    issues.push(
      makeIssue(
        ...
        "Delayed discovery — may take 2-4 weeks instead of days",
        {
          ...
          description: "Regenerate sitemap cache to include this page",
          endpoint: "/api/admin/discovery",
          payload: { action: "refresh_sitemap", siteId },
    ...
    issues.push(
      makeIssue(
        ...
        "Zero search visibility — page is invisible to all searchers",
        {
          id: "diagnose-deindex",
          label: "Diagnose & Fix",
          description: "Analyze why Google deindexed this page and attempt to fix",
          endpoint: "/api/admin/discovery",
          payload: { action: "diagnose_deindex", slug: post.slug, siteId },
    ...
    issues.push(
      makeIssue(
        ...
        "Page cannot be found in search until error is resolved",
        {
          ...
          description: "Clear error and resubmit to all engines",
          endpoint: "/api/admin/discovery",
          payload: { action: "retry_submission", slug: post.slug, siteId },
          estimatedTimeMs: 3000,
          requiresAI: false,
          destructive: false,
        },
    ...
    if (daysSinceSubmit > THRESHOLDS.staleSubmissionDays) {
      issues.push(
        makeIssue(
          ...
          {
            id: "enhance-resubmit",
            label: "Enhance & Resubmit",
            description: "Improve content quality signals and resubmit",
            endpoint: "/api/admin/discovery",
            payload: { action: "enhance_and_resubmit", slug: post.slug, siteId },
  ...
  if (
    indexRecord &&
    indexRecord.submission_attempts >= THRESHOLDS.chronicFailureAttempts &&
    indexStatus !== "indexed"
  ) {
    issues.push(
      makeIssue(
        "index-chronic-failure",
        "indexability",
        "critical",
        ...
        "Wasted crawl budget + zero return",
        {
          ...
          description: "Full technical + content analysis to find root cause",
          endpoint: "/api/admin/discovery",
          payload: { action: "deep_diagnose", slug: post.slug, siteId },
    ...
    issues.push(
      makeIssue(
        ...
        `~${Math.round((reqs.minWordCount - wordCountEn) * 0.15)}% lower chance of indexing`,
        {
          id: "expand-content",
          label: "AI Expand",
          description: `Add ${reqs.minWordCount - wordCountEn}+ words with AI while preserving voice`,
          endpoint: "/api/admin/discovery",
          payload: { action: "expand_content", slug: post.slug, siteId, targetWords: reqs.minWordCount },
  ...
  if (internalLinks < reqs.minInternalLinks) {
    issues.push(
      makeIssue(
        ...
        {
          id: "inject-internal-links",
          label: "Auto-Link",
          description: "Find and inject relevant internal links from published articles",
          endpoint: "/api/admin/discovery",
          payload: { action: "inject_internal_links", slug: post.slug, siteId },
          estimatedTimeMs: 5000,
          requiresAI: false,
          destructive: false,
        },
  ...
  if (reqs.minAffiliateLinks > 0 && affiliateLinks < reqs.minAffiliateLinks) {
    issues.push(
      makeIssue(
        ...
        {
          id: "inject-affiliates",
          label: "Add Affiliates",
          description: "Inject relevant booking/affiliate links based on content topic",
          endpoint: "/api/admin/discovery",
          payload: { action: "inject_affiliates", slug: post.slug, siteId },
          estimatedTimeMs: 8000,
          requiresAI: true,
          destructive: false,
        },
  ...
  if (contentEn.includes("TOPIC_SLUG") || contentEn.includes("PLACEHOLDER")) {
    issues.push(
      makeIssue(
        ...
        {
          id: "fix-placeholder-links",
          label: "Fix Placeholders",
          description: "Replace TOPIC_SLUG placeholders with real article links or remove them",
          endpoint: "/api/admin/discovery",
          payload: { action: "fix_placeholders", slug: post.slug, siteId },
          estimatedTimeMs: 5000,
          requiresAI: false,
          destructive: false,
        },
  ...
  if (metaTitleLen < THRESHOLDS.metaTitleMin || metaTitleLen > THRESHOLDS.metaTitleMax) {
    issues.push(
      makeIssue(
        "meta-title-length",
        "meta_tags",
        metaTitleLen === 0 ? "critical" : "medium",
        metaTitleLen === 0
          ? "Missing meta title"
          : `Meta title ${metaTitleLen} chars (optimal: ${THRESHOLDS.metaTitleMin}-${THRESHOLDS.metaTitleMax})`,
        metaTitleLen === 0
          ? "No title tag — Google will auto-generate one (usually poorly)."
          : metaTitleLen < THRESHOLDS.metaTitleMin
            ? "Title too short — not enough context for searchers."
            : "Title too long — will be truncated in search results.",
        "Lower CTR from search results",
        {
          id: "fix-meta-title",
          label: "Generate Title",
          description: "AI-generate optimal meta title based on content",
          endpoint: "/api/admin/discovery",
          payload: { action: "fix_meta_title", slug: post.slug, siteId },
          estimatedTimeMs: 5000,
          requiresAI: true,
          destructive: false,
        },
  ...
  if (metaDescLen < THRESHOLDS.metaDescMin || metaDescLen > THRESHOLDS.metaDescMax) {
    issues.push(
      makeIssue(
        "meta-desc-length",
        "meta_tags",
        metaDescLen === 0 ? "critical" : "medium",
        metaDescLen === 0
          ? "Missing meta description"
          : `Meta description ${metaDescLen} chars (optimal: ${THRESHOLDS.metaDescMin}-${THRESHOLDS.metaDescMax})`,
        metaDescLen === 0
          ? "No description — Google will pull random text from the page."
          : metaDescLen < THRESHOLDS.metaDescMin
            ? "Description too short — not compelling enough to drive clicks."
            : "Description too long — will be truncated at ~160 chars.",
        "~15-30% lower CTR from search results",
        {
          id: "fix-meta-desc",
          label: "Generate Description",
          description: "AI-generate compelling meta description",
          endpoint: "/api/admin/discovery",
          payload: { action: "fix_meta_description", slug: post.slug, siteId },
          estimatedTimeMs: 5000,
          requiresAI: true,
          destructive: false,
        },
    ...
    issues.push(
      makeIssue(
        ...
        "Diluted keyword relevance",
        {
          id: "fix-headings",
          label: "Fix H1→H2",
          description: "Demote extra H1 tags to H2",
          endpoint: "/api/admin/discovery",
          payload: { action: "fix_headings", slug: post.slug, siteId },
    ...
    issues.push(
      makeIssue(
        ...
        "Missed image search traffic + accessibility violation",
        {
          ...
          description: "AI-generate descriptive alt text for all images",
          endpoint: "/api/admin/discovery",
          payload: { action: "fix_alt_text", slug: post.slug, siteId },
          estimatedTimeMs: 8000,
          requiresAI: true,
          destructive: false,
        },
  ...
  if (aioEligibility < THRESHOLDS.aioMinEligibility && contentType === "blog") {
    issues.push(
      makeIssue(
        ...
        {
          id: "boost-aio",
          label: "Boost AIO Readiness",
          description: "Restructure intro + add question H2s + inject citable data",
          endpoint: "/api/admin/discovery",
          payload: { action: "boost_aio", slug: post.slug, siteId },
          estimatedTimeMs: 15000,
          requiresAI: true,
          destructive: false,
        },
    ...
    issues.push(
      makeIssue(
        ...
        `~${Math.round(impressions7d * 0.03 - clicks7d)} missed clicks/week`,
        {
          id: "optimize-ctr",
          label: "Optimize CTR",
          description: "AI-rewrite title and description for maximum click appeal",
          endpoint: "/api/admin/discovery",
  ...
  if (trend === "declining" && impressions30d > 50) {
    issues.push(
      makeIssue(
        ...
        {
          id: "diagnose-decline",
          label: "Diagnose Decline",
          description: "Analyze ranking drop causes and suggest improvements",
          endpoint: "/api/admin/discovery",
          payload: { action: "diagnose_decline", slug: post.slug, siteId },
          estimatedTimeMs: 10000,
          requiresAI: true,
          destructive: false,
        },
    ...
    issues.push(
      makeIssue(
        ...
        "Gradual ranking decay",
        {
          ...
          description: "Update dates, add new information, refresh statistics",
          endpoint: "/api/admin/discovery",
          payload: { action: "refresh_content", slug: post.slug, siteId },
          estimatedTimeMs: 15000,
          requiresAI: true,
          destructive: false,
        },
  ...
  if (reqs.requireAuthenticitySignals > 0 && authenticityScore < 50) {
    issues.push(
      makeIssue(
        ...
        {
          id: "boost-authenticity",
          label: "Add Experience Signals",
          description: "Inject first-hand experience markers, sensory details, and insider tips",
          endpoint: "/api/admin/discovery",
          payload: { action: "boost_authenticity", slug: post.slug, siteId },
          estimatedTimeMs: 15000,
          requiresAI: true,
          destructive: false,
        },
    ...
    issues.push(
      makeIssue(
        ...
        "Weaker E-E-A-T signals → lower rankings",
        {
          id: "add-author",
          label: "Add Author",
          description: "Assign a team member author from the rotation",
          endpoint: "/api/admin/discovery",
          payload: { action: "add_author", slug: post.slug, siteId },
          estimatedTimeMs: 2000,
          requiresAI: false,
          destructive: false,
        },
...
function buildSummary(
  ...
  const crawlabilityScore = Math.round(
    (inSitemap / Math.max(1, published)) * 50 + (submitted / Math.max(1, published)) * 50,
  );
  ...
  const topIssues = allIssues.sort((a, b) => severityOrder[a.severity] - severityOrder[b.severity]).slice(0, 10);
