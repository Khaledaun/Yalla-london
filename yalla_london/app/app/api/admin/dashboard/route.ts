  ...
  try {
    ...
    const connectionStates: ConnectionState = {
      analytics: {
        ...
        error: analyticsConfig?.test_status === 'failed' ? 'Connection failed' : undefined
      },
      searchConsole: {
        connected: !!searchConsoleConfig?.key_value,
        service: 'Google Search Console',
        lastSync: searchConsoleConfig?.last_tested?.toISOString(),
        error: searchConsoleConfig?.test_status === 'failed' ? 'Connection failed' : undefined
      },
      wordpress: {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        recentActivity: recentActivity.map((activity: any) => ({
          ...
          description: activity.entity_id ? `Entity: ${activity.entity_id}` : 'System action',
    ...
    return NextResponse.json(
      {
        ...
        fallback: {
          ...
          taskSummary: { total: 0, pending: 0, inProgress: 0, overdue: 0, highPriority: 0 },
          pipelineHealth: {
            contentPipeline: { status: 'error' as const, articlesInDraft: 0, articlesInReview: 0, scheduledContent: 0 },
            seoHealth: { status: 'error' as const, averageScore: 0, issuesCount: 0 },
            systemHealth: { status: 'error' as const, uptime: 0, lastBackup: 'Unknown' }
          },
          connectionStates: {
            analytics: { connected: false, service: 'Google Analytics 4', error: 'Service unavailable' },
            searchConsole: { connected: false, service: 'Google Search Console', error: 'Service unavailable' },
            wordpress: { connected: false, service: 'WordPress', error: 'Service unavailable' }
          },
          recentActivity: [],
          lastUpdated: new Date().toISOString(),
          cached: false
