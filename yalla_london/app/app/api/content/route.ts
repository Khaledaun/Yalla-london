...
const ContentQuerySchema = z.object({
  ...
  page_type: z.enum(['guide', 'place', 'event', 'list', 'faq', 'news', 'itinerary', 'comparison', 'review', 'listicle', 'how-to']).optional(),
...
function transformStaticPost(post: typeof allStaticPosts[0], locale: 'en' | 'ar') {
  ...
  try {
    ...
    try {
      ...
      switch (sort) {
        ...
        case 'popular': orderBy = { seo_score: 'desc' }; break;
        case 'seo_score': orderBy = { seo_score: 'desc' }; break;
    ...
    if (useStaticContent) {
      ...
      if (category) {
        ...
        if (categoryObj) {
          filteredPosts = filteredPosts.filter(p => p.category_id === categoryObj.id);
        }
      }
      ...
      switch (sort) {
        case 'oldest':
          filteredPosts.sort((a, b) => a.created_at.getTime() - b.created_at.getTime());
          break;
        case 'seo_score':
        case 'popular':
