...
export default function PurchasesPage() {
  ...
  const statusIcon = (status: string) => {
    switch (status) {
      case 'COMPLETED': return <Check className="w-4 h-4 text-forest" />
      case 'PENDING': return <Clock className="w-4 h-4 text-yalla-gold-500" />
      default: return <AlertCircle className="w-4 h-4 text-london-600" />
    }
  }
  ...
  return (
    <div className="min-h-screen bg-cream" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-center p-4 pointer-events-none">
        <nav className="flex items-center justify-between px-6 py-3 bg-white/95 backdrop-blur-xl rounded-full shadow-lg w-full max-w-4xl pointer-events-auto border border-sand">
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link href="/shop" className="text-sm font-medium text-stone hover:text-charcoal">Shop</Link>
            <Link href="/shop/purchases" className="text-sm font-medium text-charcoal">My Purchases</Link>
          </div>
        </nav>
      </div>
      ...
      <div className="pt-28 pb-16 max-w-2xl mx-auto px-6">
        ...
        <form onSubmit={lookupPurchases} className="flex gap-3 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 border border-sand rounded-xl focus:outline-none focus:ring-2 focus:ring-london-600/20 focus:border-london-600"
            />
          </div>
          <button
            type="submit"
            disabled={loading}
            className="px-6 py-3 bg-london-600 hover:bg-london-700 text-white font-semibold rounded-xl transition-colors disabled:opacity-50"
          >
        ...
        {error && (
          <div className="p-4 bg-london-50 text-london-600 rounded-xl mb-6">{error}</div>
        )}
          ...
          <>
            {purchases.length === 0 ? (
              <div className="text-center py-12">
                <Package className="w-16 h-16 mx-auto mb-4 text-stone" />
                ...
                <Link href="/shop" className="text-london-600 hover:underline font-medium">
              ...
              <div className="space-y-4">
                {purchases.map((purchase) => (
                  <div key={purchase.id} className="bg-white rounded-xl border border-sand shadow-sm p-5">
                    <div className="flex items-start gap-4">
                      {/* Icon */}
                      <div className="w-12 h-12 bg-london-600/10 rounded-lg flex items-center justify-center flex-shrink-0">
                      ...
                      {purchase.downloadUrl && purchase.downloadsRemaining > 0 && (
                        <a
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-london-600 hover:bg-london-700 text-white font-medium rounded-lg transition-colors flex-shrink-0"
                        >
